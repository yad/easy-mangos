        ...
        for (SpellAuraHolderMap::iterator iter = spair.first; iter != spair.second; ++iter)
        {
            ...
            if (const SpellEntry *sp = foundHolder->GetSpellProto())
            {
                switch (sp->SpellFamilyName)
                {
                    ...
                    case SPELLFAMILY_PRIEST:
                        // Mind Flay
                        //if (sp->SpellIconID == 548 && ClassFamilyMask(sp->SpellFamilyFlags, 0x00000040))
                        //    continue;
...
Unit* Unit::SelectRandomUnfriendlyTargetWithBgCheck(Unit* except /*= NULL*/, float radius /*= ATTACK_DISTANCE*/) const
{
    std::list<Unit *> targets;
...
bool Unit::FindRealPlayerInRange(float radius /*= ATTACK_DISTANCE*/) const
{
    std::list<Unit *> targets;
    ...
    for(std::list<Unit *>::iterator tIter = targets.begin(); tIter != targets.end();)
    {
        ...
        else if( ((*tIter)->GetTypeId()!=TYPEID_PLAYER) || (((*tIter)->GetTypeId()==TYPEID_PLAYER) && ((Player*)*tIter)->IsBot()))
        {
            std::list<Unit *>::iterator tIter2 = tIter;
            ++tIter;
            targets.erase(tIter2);
...
//    SendMonsterMove(x, y, z, SPLINETYPE_NORMAL, SplineFlags(SPLINEFLAG_TRAJECTORY | SPLINEFLAG_WALKMODE), transitTime, NULL, double(verticalSpeed));
    MonsterMoveWithSpeed(x, y, z, 28.0f);
/*
    if (GetTypeId() != TYPEID_PLAYER)
    {
