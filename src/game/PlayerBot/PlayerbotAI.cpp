...
bool PlayerbotAI::HasAura(uint32 spellId, const Unit* target) const
{
    if (spellId <= 0)
        return false;

    for (Unit::SpellAuraHolderMap::const_iterator iter = target->GetSpellAuraHolderMap().begin(); iter != target->GetSpellAuraHolderMap().end(); ++iter)
    {
...
bool PlayerbotAI::HasAura(const char* spellName, const Unit* target) const
{
    uint32 spellId = getSpellId(spellName);
    return (spellId) ? HasAura(spellId, target) : false;
}
...
void PlayerbotAI::GetCombatTarget(Unit* forcedTarget)
{
    ...
    m_bot->SetSelectionGuid(ObjectGuid(m_targetCombat->GetObjectGuid()));
    m_ignoreAIUpdatesUntilTime = time(0) + 1;
...
void PlayerbotAI::UpdateAttackerInfo()
{
    ...
    //sLog.outDebug( "[PlayerbotAI]: %s m_attackerInfo = {", m_bot->GetName() );
    //for( AttackerInfoList::iterator i=m_attackerInfo.begin(); i!=m_attackerInfo.end(); ++i )
    //    sLog.outDebug( "[PlayerbotAI]:     [%016I64X] { %08X, %08X, %.2f, %.2f, %d, %d }",
    //        i->first,
...
uint32 PlayerbotAI::EstRepair(uint16 pos)
{
    ...
    if (LostDurability > 0)
    {
        ...
        if (!dcost)
        {
            sLog.outDebug("RepairDurability: Wrong item lvl %u", ditemProto->ItemLevel);
            return TotalCost;
        }
        ...
        if (!dQualitymodEntry)
        {
            sLog.outDebug("RepairDurability: Wrong dQualityModEntry %u", dQualitymodEntryId);
            return TotalCost;
        }
...
//        m_targetProtect = 0;
    ...
    {
        // sLog.outDebug("Selected link : %s", text.c_str());
