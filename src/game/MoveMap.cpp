...
void Map::LoadNavMesh(int gx, int gy)
{
    ...
    sLog.outError("Loaded tile %03i(%u)[%02i,%02i]  into [%02i,%02i]", i_id, GetInstanceId(), gx, gy, header->x, header->y);
}
...
void Map::UnloadNavMesh(int gx, int gy)
{
    ...
    sLog.outError("Unloaded tile %03i(%u)[%02i,%02i]", i_id, GetInstanceId(), gx, gy);
}
