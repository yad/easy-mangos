...
int32 OutdoorPvPObjectiveZM_GraveYard::HandleOpenGo(Player *plr, uint64 guid)
{
    ...
    if(retval>=0)
    {
        if(plr->HasAura(ZM_BATTLE_STANDARD_A,EFFECT_INDEX_0) && m_GraveYardState != ZM_GRAVEYARD_A)
        {
        ...
        else if(plr->HasAura(ZM_BATTLE_STANDARD_H,EFFECT_INDEX_0) && m_GraveYardState != ZM_GRAVEYARD_H)
        {
